...
namespace PortfolioManager.Classes
{
    class DigitalOption:Options
    {
        ...
        public DigitalOption(String issuer, Exchange[] tradedOnExchange, string symbol, ISecurity underlying, DateTime expiryDate, double strikePrice, double historicalVolatility, String isin, Boolean isTradable, OptionType type, OptionKind optionKind, Double rebate)
        {
            this.issuer = issuer;
        ...
        public void calculateStandardError(long numberOfSimulations, Double[] priceAtEnd, Double daysToExpiry, Boolean antitheticReduction)
        {
            ...
            for (int i = 0; i < priceAtEnd.Length; i++)
            {
                if (this.type == OptionType.CALL)
                {
                    sum += Math.Pow(((priceAtEnd[i] >= this.strikePrice ? this.rebate : 0) * Math.Exp(-Simulator.yieldCurve[0].Rate * daysToExpiry / 365.0)) - this.Price, 2);
                }
                else if (this.type == OptionType.PUT)
                {
                    sum += Math.Pow(((priceAtEnd[i] < this.strikePrice ? this.rebate : 0) * Math.Exp(-Simulator.yieldCurve[0].Rate * daysToExpiry / 365.0)) - this.Price, 2);
        ...
        public void calculateStandardErrorwithcv(long numberOfSimulations, Double[] priceAtEnd, Double daysToExpiry, Boolean antitheticReduction, Double[] controlVariateList)
        {
            ...
            for (int i = 0; i < priceAtEnd.Length; i++)
            {
                if (this.type == OptionType.CALL)
                {
                    payoff[i] = (priceAtEnd[i] >= this.strikePrice ? this.rebate : 0);
                }
                else
                {
                    payoff[i] = (priceAtEnd[i] < this.strikePrice ? this.rebate : 0);
            ...
            for (int i = 0; i < priceAtEnd.Length; i++)
            {
                if (this.type == OptionType.CALL)
                {
                    sum2 += Math.Pow((priceAtEnd[i] >= this.strikePrice ? this.rebate : 0) - beta * controlVariateList[i], 2);
                }
                else if (this.type == OptionType.PUT)
                {
                    sum2 += Math.Pow((priceAtEnd[i] < this.strikePrice ? this.rebate : 0) - beta * controlVariateList[i], 2);
