...
{
    class LookbackOption:Options
    {
        #region Constructors of the European option
        public LookbackOption(string symbol, ISecurity underlying, DateTime expiryDate, double strikePrice, double historicalVolatility, OptionType type, OptionKind optionKind)
        {
            this.symbol = symbol;
        ...
        public LookbackOption(String issuer, Exchange[] tradedOnExchange, string symbol, ISecurity underlying, DateTime expiryDate, double strikePrice, double historicalVolatility, String isin, Boolean isTradable, OptionType type, OptionKind optionKind)
        {
            this.issuer = issuer;
        ...
        public override Double calculateOptionPrice(long numberOfSimulations, int numberOfDays, Double daysToExpiry, Double del = 0, ChangeValue change = ChangeValue.NONE, Boolean controlVariateReduction = false, Boolean multithreading = false)
        {
            ...
            for (int i = 0; i < priceAtEnd.Length; i++)
            {
                ...
                {
                    sum = controlVariateReduction ? (sum + Math.Max(priceAtEnd[i] - this.strikePrice, 0) - Simulator.controlVariateList[i]) : (sum + Math.Max(priceAtEnd[i] - this.strikePrice, 0));
                }
                else if (this.type == OptionType.PUT)
                {
                    sum = controlVariateReduction ? (sum + Math.Max(this.strikePrice - priceAtEnd[i], 0) - Simulator.controlVariateList[i]) : (sum + Math.Max(this.strikePrice - priceAtEnd[i], 0));
                }
            }
        ...
        public void calculateStandardError(long numberOfSimulations, Double[] priceAtEnd, Double daysToExpiry, Boolean antitheticReduction)
        {
            ...
            for (int i = 0; i < priceAtEnd.Length; i++)
            {
                if (this.type == OptionType.CALL)
                {
                    sum += Math.Pow((Math.Max(priceAtEnd[i] - this.strikePrice, 0) * Math.Exp(-Simulator.yieldCurve[0].Rate * daysToExpiry / 365.0)) - this.Price, 2);
                }
                else if (this.type == OptionType.PUT)
                {
                    sum += Math.Pow((Math.Max(this.strikePrice - priceAtEnd[i], 0) * Math.Exp(-Simulator.yieldCurve[0].Rate * daysToExpiry / 365.0)) - this.Price, 2);
                }
            }
